...
def _validate_theme_catalog(catalog: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    ...
    for key in FORBIDDEN_MEMBERSHIP_KEYS:
        if f'"{key}"' in serialized:
            raise ResearchEligibilityError("ticker/company membership is forbidden in theme catalog")
    ...
    if any(int(limits.get(key) or 0) < 1 for key in ("active_intelligence", "supply_chain", "deep_research")):
    ...
    if any(
        ...
        raise ResearchEligibilityError("theme catalog tier minimum scores must be between zero and one")
    return themes


def _catalog_matches(knowledge_ids: set[str], themes: list[Mapping[str, Any]]) -> list[Mapping[str, Any]]:
    matches = []
    for theme in themes:
...
def build_research_eligibility(
    ...
    *,
    ...
    for source in knowledge_payload["records"]:
        ...
        scores = {dimension: _max_score(knowledge, dimension) for dimension in ("theme", "sector", "cluster", "product", "supply_chain_role")}
        ...
        allowed_actions = {
            action: any(bool((theme.get("actions") or {}).get(action)) for theme in matched_catalog_themes)
            for action in ("news", "etf", "supply_chain", "deep_research")
        }
        catalog_priority = max((float(theme.get("priority") or 0) for theme in matched_catalog_themes), default=0.0)
        ...
        news_missing = []
        if not allowed_actions["news"]: news_missing.append("NO_ENABLED_CATALOG_THEME")
        ...
        etf_missing = []
        if not allowed_actions["etf"]: etf_missing.append("NO_ENABLED_CATALOG_THEME")
        ...
        chain_missing = []
        if not allowed_actions["supply_chain"]: chain_missing.append("NO_ENABLED_CATALOG_THEME")
        ...
        if max(scores["supply_chain_role"], scores["cluster"], scores["product"]) < float(thresholds["supply_chain"]["minimum_role_or_cluster_score"]): chain_missing.append("ROLE_CLUSTER_OR_PRODUCT_SCORE_BELOW_THRESHOLD")
        deep_missing = []
        if not allowed_actions["deep_research"]: deep_missing.append("NO_ENABLED_CATALOG_THEME")
        ...
        records.append({
            ...
            "instrument_scope": {
                ...
                "reason_code": "OPERATING_COMPANY_COMMON_EQUITY" if listed.get(company_id, False) else "NON_COMPANY_OR_NON_COMMON_EQUITY_EXCLUDED",
            },
            "research_score": research_score,
            "catalog_priority": catalog_priority,
            "tier_rank_score": tier_rank_score,
            "matched_catalog_theme_ids": sorted(str(theme["theme_id"]) for theme in matched_catalog_themes),
    ...
    candidates = [row for row in records if any(item["qualified"] for item in row["decisions"].values())]
    rank_key = lambda row: (-float(row["tier_rank_score"]), -float(row["research_score"]), row["company_id"])
    ...
    active_ids = {row["company_id"] for row in active_candidates[:int(limits["active_intelligence"])]}
    chain_ids = {row["company_id"] for row in chain_candidates[:int(limits["supply_chain"])]}
    deep_ids = {row["company_id"] for row in deep_candidates[:int(limits["deep_research"])]}
    ...
    for row in records:
        ...
        action_membership = {"news": active_ids, "etf": active_ids, "supply_chain": chain_ids, "deep_research": deep_ids}
        for action, decision in row["decisions"].items():
            decision["enabled"] = bool(decision["qualified"] and row["company_id"] in action_membership[action])
            if decision["qualified"] and not decision["enabled"]:
                decision["reason_code"] = "ACTION_TIER_RANK_LIMIT_EXCEEDED"
