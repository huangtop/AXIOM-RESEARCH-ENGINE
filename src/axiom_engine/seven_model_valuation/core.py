...
def calculate_seven_models(
    ...
    *,
    ...
    # Keep fiscal-transition growth separate from the dedicated PEG valuation
    # growth. New callers should provide normalized_peg_growth; legacy callers
    # remain supported through forward_eps_growth fallback.
    forward_eps_growth = _value(estimates, "forward_eps_growth")
    normalized_peg_growth = _value(estimates, "normalized_peg_growth")
    ...
    peg_value = product([
        forward_eps,
        peg_growth * Decimal("100") if peg_growth is not None else None,
        target_peg,
    ])
    ...
    return {
        ...
        "peg": _result(
            peg_value,
            "peg-fair-value.v031v.5",
            ["forward_eps", peg_growth_source, "target_peg"],
            [
                name
                for name, value in (
                    ("forward_eps", forward_eps),
                    (peg_growth_source, peg_growth),
                    ("target_peg", target_peg),
                )
                if value is None
            ],
            assumption_source="knowledge.valuation_assumptions",
        ),
