...
import json
import pytest
from axiom_engine.real_100_estimate_loader import (
...
def registry(root, n=100):
    ...
    write(root / "companies.json", companies)
    write(root / "securities.json", securities)
...
def test_write_and_validate(tmp_path):
    ...
    report = build_real_100_estimates(
        s, registry_dir=r, output_dir=out, diagnostics_file=tmp_path / "diag.json", write=True
    )
...
def test_unknown_ticker_rejected(tmp_path):
    ...
    s = tmp_path / "source.json"
    write(
        s, {"estimates": [{"ticker": "BAD", "metric": "revenue", "value": 1, "fiscal_year": 2027}]}
    )
    with pytest.raises(Real100EstimateError, match="unknown company"):
...
def test_duplicate_key_rejected(tmp_path):
    ...
    with pytest.raises(Real100EstimateError, match="duplicate"):
        build_real_100_estimates(s, registry_dir=r)
...
def test_partially_filled_row_remains_an_error(tmp_path):
    r = tmp_path / "registry"
    registry(r, 1)
    s = tmp_path / "partial.csv"
    s.write_text(
        "company_id,ticker,metric,value,unit,currency,period_end,fiscal_year,fiscal_period,estimate_kind,analyst_count,source_record_id\ncompany:US-000,T000,revenue,,currency,USD,2027-12-31,2027,FY,consensus_mean,,\n",
        encoding="utf-8",
    )
...
def test_template_prefills_forward_period(tmp_path):
    ...
    template = tmp_path / "template.csv"
    build_real_100_estimate_template(
        registry_dir=r, output=template, fiscal_year=2028, period_end="2028-09-30"
    )
...
def test_cli_template_exposes_forward_period_options():
    ...
    command = get_command(app).commands["build-real-100-estimate-template"]
    options = {option for parameter in command.params for option in parameter.opts}
    assert {"--fiscal-year", "--period-end"}.issubset(options)
...
def test_cli_build_exposes_provider_and_compact_options():
    ...
    command = get_command(app).commands["build-real-100-estimates"]
    options = {option for parameter in command.params for option in parameter.opts}
    assert {
        "--compact",
        "--adapter",
